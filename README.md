...
- **`K_r` refresh / double-ratchet** (`~DECISION`): **resolved**: the spec implements a rotation-gate ratchet: each generation derives `K_{g+1} = HMAC(K_g, "fre:v1:ratchet" ‖ FPR ‖ le64(g))`, `K_0 = K_r`, and `rotate` returns both `S_{g+1}` and `K_{g+1}` (R11). An erased root (R12) means a `K_r` leak is no longer a permanent master key. A kept root remains a master, so erasure is a SHOULD that every integration should honor.
...
- **Low-entropy floor**: **resolved**. Reversal is `min(2^entropy(input), 2^λ)` and the risk is *documented only* (see the security-model WARNING): no forced minimum work, caller pads or diversifies. If a use case needs a floor, revisit `FRE_HARD` then.
...
- **Reference implementation language**: **resolved** **Rust** (`src/`, reference, vectors generated here) **and C** (`c/`, dep-free embedded-portable port with a vendored SHA-256/HMAC). Both pass the pinned vectors above; the vendored HMAC is additionally conformance-tested against RFC 4231. `scripts/check.sh` runs `cargo test` + `clippy`, `cargo audit` when installed, the C tests, an ASan/UBSan build of the C tests, and a deterministic Rust <=> C differential harness (`examples/difftest.rs` piped into `c/difftest`) that cross-checks `fre_fpr`/`fre_rotate`/`fre_encode` over ~20k adversarial cases. A Python generator/port could be added later on the same vectors. The crate publishes to crates.io as **`rust-fre`**, packaging only `src/`, `examples/` and `README.md`; the C port stays in-repo (never published) and pairs with the crate by directory.
